...
public class PriorityController {
    ...
    public ResponseEntity<Priority> add(@RequestBody Priority priority, @AuthenticationPrincipal Jwt jwt) {
        ...
        priority.setEmail(jwt.getClaimAsString("email")); // email пользователя из KeyCloak
...
//        }
    ...
    @PutMapping("/update")
    public ResponseEntity update(@RequestBody Priority priority,  @AuthenticationPrincipal Jwt jwt) {
    ...
    public ResponseEntity<List<Priority>> search(@RequestBody PrioritySearchValues prioritySearchValues, @AuthenticationPrincipal Jwt jwt) {
        ...
        prioritySearchValues.setEmail(jwt.getClaimAsString("email"));

        // проверка на обязательные параметры
        if (prioritySearchValues.getEmail().isBlank()) {
            return new ResponseEntity("missed param: email", HttpStatus.NOT_ACCEPTABLE);
        }

        // если вместо текста будет пусто или null - вернутся все категории
        return ResponseEntity.ok(priorityService.findByTitle(prioritySearchValues.getTitle(), prioritySearchValues.getEmail()));
    }

...
public interface TaskRepository extends JpaRepository<Task, Long> {
    ...
    @Query("SELECT t FROM Task t where " +
            ...
            "(:categoryId is null or t.category.id=:categoryId) and " +
            "(" +
            "(coalesce(:dateFrom) is null or t.taskDate>=:dateFrom) and " +
            "(coalesce(:dateTo) is null or t.taskDate<=:dateTo)" +
            ") and " +
            "(t.email=:email)" // показывать задачи только определенного пользователя, а не все
    )
    ...
    List<Task> findByEmailOrderByTitleAsc(String email);
